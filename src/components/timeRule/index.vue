  ...
  <div class="time-rule">
    <div class="time-day" ref="day" :style="{ left: timeDayX + 'px' }">
      <div
        ...
        v-for="(n, index) in 1440 / minutesPerUnit"
        :key="index + 'out'"
        @click.prevent="clickMinute((n - 1) * minutesPerUnit)"
      ></div>
      <div
        :class="[n == 1 ? 'time-text-first' : 'time-text']"
        v-for="n in 24"
        :key="n"
      >
        {{ hourText(n - 1) }}
...
export default {
  ...
  methods: {
    ...
    triggerTimeChange() {
      this.$emit("timeChange", this.activeMinutes[this.timeCursorText], true);
    },
  },
  computed: {
    timeCursorText: {
      ...
      set() {
        ...
        if (
          this.timeDayX <
          $(this.$el).innerWidth() - $(this.$refs.day).outerWidth()
        ) {
          this.timeDayX =
            $(this.$el).innerWidth() - $(this.$refs.day).outerWidth();
        }
    ...
    activeMinutes() {
      ...
      for (let video of this.videos) {
        // debugger;
        // let start = moment(video.startTime, "YYYYMMDDHHmmss");
        let m = moment(video.startTime, "YYYYMMDDHHmmss");
